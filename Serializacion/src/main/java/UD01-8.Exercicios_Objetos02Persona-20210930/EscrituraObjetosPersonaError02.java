...
/*
 * Ejemplo que a�ade una segunda cabecera en el fichero de objetos
 * si el fichero lo creamos en el modo a�adir 
 * NESTA SITUACI�N, SE EXECUTAMOS O FICHEIRO VARIAS VECES..
 * CREA� NO MEDI DO FICHEIRO UNHA CABECEIRA POR CADA EXECUCI�N
 * DANDO LUGAR A ERROS NA LECTURA.
 * NO PAQUETE ejemplos09ObjetosAlumnos QUE VEREMOS DESEGUIDO temos detallado un exemplo
 * DE COMO SOLUCIONAR ESTA SITUACI�N.
 *  
 *  */
...
public class EscrituraObjetosPersonaError02 {
	...
	public static void Escritura(){
		...
		try{
			...
			flujoescritura.writeObject( new Persona ("Pepe Salas P�rez", 45));
			flujoescritura.writeObject( new Persona ("Maria Mart�n Sierra", 23));*/
			
			flujoescritura = new ObjectOutputStream ( new FileOutputStream (archivo, true));//FileOutputStream cun segundo par�metro a true, SOBRESCRIBE.
			...
			try {
				if (flujoescritura!=null){
					...
					System.out.println("Fin de la inserci�n de registros");
				}
			}catch(IOException e){
